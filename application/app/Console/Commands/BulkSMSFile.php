...
class BulkSMSFile extends Command
{
   ...
   public function process($text)
    ...
    public function replace($text)
    {
        ...
        public function rotateSenderId($gateway) {

            $ids_ = explode(',', $gateway->gateway_ids);
            $random_number = mt_rand(0, count($ids_)-1);
            $sender_id_ran = $ids_[$random_number];
    
            if($sender_id_ran == $this->recentlyUsedId) {
                $this->rotateSenderId($gateway);
            }
                ...
                if ($list_count > 0) {
                    ...
                    foreach ($subscription_list as $list) {
                        ...
                        if ($msg_type == 'plain' || $msg_type == 'unicode' || $msg_type == 'arabic') {
                            dispatch(new SendBulkSMS($camp->user_id, $list->number, $gateway, $gateway_credential, $camp->sender ? $camp->sender : $this->rotateSenderId($gateway), $this->process($list->message), $msgcount, $cg_info, '', $msg_type, $list->id));
                        }

                        if ($msg_type == 'voice') {
                            dispatch(new SendBulkVoice($camp->user_id, $list->number, $gateway, $gateway_credential, $camp->sender ? $camp->sender : $this->rotateSenderId($gateway), $this->process($list->message), $msgcount, '', $msg_type, $list->id));
                        }


                        if ($msg_type == 'mms') {
                            $media_url = $camp->media_url;
                            dispatch(new SendBulkMMS($camp->user_id, $list->number, $gateway, $gateway_credential, $camp->sender ? $camp->sender : $this->rotateSenderId($gateway), $this->process($list->message),$media_url, '', $msg_type, $list->id));
                        }
                    }
